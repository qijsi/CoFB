...
static gro_tbl_destroy_fn tbl_destroy_fn[RTE_GRO_TYPE_MAX_NUM] = {
    gro_tcp4_tbl_destroy, gro_vxlan_tcp4_tbl_destroy, NULL};
static gro_tbl_pkt_count_fn tbl_pkt_count_fn[RTE_GRO_TYPE_MAX_NUM] = {
    gro_tcp4_tbl_pkt_count, gro_vxlan_tcp4_tbl_pkt_count, NULL};
...
#define IS_IPV4_VXLAN_TCP4_PKT(ptype)                              \
  (RTE_ETH_IS_IPV4_HDR(ptype) &&                                   \
   ...
   (((ptype & RTE_PTYPE_INNER_L3_MASK) &                           \
     (RTE_PTYPE_INNER_L3_IPV4 | RTE_PTYPE_INNER_L3_IPV4_EXT |      \
...
void *rte_gro_ctx_create(const struct rte_gro_param *param) {
  ...
  if (gro_ctx == NULL) return NULL;

  for (i = 0; i < RTE_GRO_TYPE_MAX_NUM; i++) {
    gro_type_flag = 1ULL << i;
    if ((param->gro_types & gro_type_flag) == 0) continue;

    create_tbl_fn = tbl_create_fn[i];
    if (create_tbl_fn == NULL) continue;
...
void rte_gro_ctx_destroy(void *ctx) {
  ...
  for (i = 0; i < RTE_GRO_TYPE_MAX_NUM; i++) {
    gro_type_flag = 1ULL << i;
    if ((gro_ctx->gro_types & gro_type_flag) == 0) continue;
    destroy_tbl_fn = tbl_destroy_fn[i];
    if (destroy_tbl_fn) destroy_tbl_fn(gro_ctx->tbls[i]);
  }
  rte_free(gro_ctx);
}
...
uint16_t rte_gro_reassemble_burst(struct rte_mbuf **pkts, uint16_t nb_pkts,
  ...
  if (unlikely((param->gro_types &
                (RTE_GRO_IPV4_VXLAN_TCP_IPV4 | RTE_GRO_TCP_IPV4)) == 0))
    return nb_pkts;
  ...
  for (i = 0; i < nb_pkts; i++) {
     ...
     */
    if (IS_IPV4_VXLAN_TCP4_PKT(pkts[i]->packet_type) && do_vxlan_gro) {
  ...
  if (nb_after_gro < nb_pkts) {
    ...
    if (do_vxlan_gro) {
      i = gro_vxlan_tcp4_tbl_timeout_flush(&vxlan_tbl, 0, pkts, nb_pkts);
    }
    if (do_tcp4_gro) {
...
uint16_t rte_gro_dplane_reassemble(struct rte_mbuf **pkts, uint16_t nb_pkts,
                                   void *ctx, uint64_t start,
                                   Skiplist *skiplist,
  ...
  for (i = 0; i < nb_pkts; i++) {
    if (IS_IPV4_VXLAN_TCP4_PKT(pkts[i]->packet_type) && do_vxlan_gro) {
      if (gro_vxlan_tcp4_reassemble(pkts[i], vxlan_tbl, current_time) < 0)
        unprocess_pkts[unprocess_num++] = pkts[i];
    } else if (IS_IPV4_TCP_PKT(pkts[i]->packet_type) && do_tcp4_gro) {
      ret = gro_tcp4_dplane_reassemble(pkts[i], tcp_tbl, current_time, htable,
                                       &flow);
      if (ret < 0) {
        //   printf("gro_tcp4_dplane_reassemble return < 0\n");
        unprocess_pkts[unprocess_num++] = pkts[i];
      } else {
        //  flow->num++;
        if (ret == 2) {
          if (!skiplistSearchValue(skiplist, (flow->start_time - start),
                                   flow->fdir_id))
            skiplistAdd(skiplist, (flow->start_time - start), flow->fdir_id);
#if 0
          printf("add to skiplist, fdir_id:%u length:%d time:%lu\n", \
          flow->fdir_id, skiplist->length, (flow->start_time - start)/1000);
          fflush(stdout);
#endif
          // flow->state = RECV;
        }
      ...
      printf("cannot implement merge logic\n");
      unprocess_pkts[unprocess_num++] = pkts[i];
    }
  }
  if (unprocess_num > 0) {
...
#if 0
      if (NULL != flow)
        printf("after tcp4_dplane_reassemble, state:%u length:%u\n",
               flow->state, flow->size);
        else
          printf("flow is null\n");
#endif
        if (ret < 0) {
          printf("gro_tcp4_dplane_reassemble return < 0\n");
          unprocess_pkts[unprocess_num++] = pkts[i];
        } else if (ret == 2 || (flow->state == FLUSHABLE)) {
          skiplistAdd(skiplist, (flow->start_time - start) / 1000,
                      flow->fdir_id);
          printf("add to skiplist, fdir_id:%u length:%d time:%lu\n", \
          flow->fdir_id, skiplist->length, (flow->start_time - start)/1000);

#if 0
          skiplistAdd(skiplist, (((struct gro_tcp4_tbl *)tcp_tbl)
                                                   ->flows[finfo.flow_idx]
                                                   .start_time -
                                               start) /
                                                  1000,
                                              fdir_hash);
...
uint32_t rte_gdp_reassemble(gdp_flowtable *ht, struct rte_mbuf **pkts,
                            uint16_t nb_pkts, priority_queue queue) {
  int i;
  uint32_t fdir_id;
  uint32_t state;
  for (i = 0; i < nb_pkts; i++) {
    struct rte_mbuf *mbuf = pkts[i];
    state = GDPInsertPkt(ht, mbuf->hash.fdir.hi, pkts[i]);
    if (state == NEW) {
      QueueInsert(queue, fdir_id);
    }
  }
}

uint32_t rte_gdp_fetchflow(gdp_flowtable *ht, priority_queue queue, struct rte_mbuf **out) {
  uint32_t fdir_id;
  uint32_t pending_mbuf;
  uint32_t i;
  uint32_t num = 0;
  flow_elem *item;
  struct rte_mbuf *mbuf_chain;
  for (item = TAILQ_FIRST(&queue.head[RECV]); item != NULL;
       item = TAILQ_NEXT(item, next)) {
    fdir_id = item->fdir_id;
    mbuf_chain = ht->table[fdir_id].head;
    pending_mbuf = ht->table[fdir_id].info.pending_mbuf;
    for (i = 0; i < pending_mbuf; i++) {
      out[num + i] = GDPIssuePkt(ht, fdir_id);
    }
    num += i;
  }
  return num;
...
uint16_t rte_gro_reassemble(struct rte_mbuf **pkts, uint16_t nb_pkts,
  ...
  for (i = 0; i < nb_pkts; i++) {
    if (IS_IPV4_VXLAN_TCP4_PKT(pkts[i]->packet_type) && do_vxlan_gro) {
      ...
      ret = gro_tcp4_reassemble(pkts[i], tcp_tbl, current_time);
      if (ret < 0) unprocess_pkts[unprocess_num++] = pkts[i];
    } else {
      printf("cannot implement merge logic\n");
      unprocess_pkts[unprocess_num++] = pkts[i];
    }
...
uint16_t rte_gro_dplane_flush(void *ctx, uint64_t gro_types,
                              struct rte_mbuf **out, uint16_t max_nb_out,
                              struct fdir_hashtable *htable, uint32_t fdir_id) {
  struct gro_ctx *gro_ctx = ctx;
  uint64_t flush_timestamp;
  uint16_t num = 0;

  gro_types = gro_types & gro_ctx->gro_types;
  flush_timestamp = rte_rdtsc();
  ...
  if ((gro_types & RTE_GRO_TCP_IPV4) && max_nb_out > 0) {
    num += gro_tcp4_tbl_dplane_flush(gro_ctx->tbls[RTE_GRO_TCP_IPV4_INDEX],
                                     &out[num], max_nb_out, htable, fdir_id);
  }
...
uint16_t rte_gro_timeout_flush(void *ctx, uint64_t timeout_cycles,
  ...
  if ((gro_types & RTE_GRO_TCP_IPV4) && max_nb_out > 0) {
    num += gro_tcp4_tbl_timeout_flush(gro_ctx->tbls[RTE_GRO_TCP_IPV4_INDEX],
                                      flush_timestamp, &out[num], max_nb_out);
  }
...
uint64_t rte_gro_get_pkt_count(void *ctx) {
  ...
  for (i = 0; i < RTE_GRO_TYPE_MAX_NUM && gro_types; i++) {
    flag = 1ULL << i;
    if ((gro_types & flag) == 0) continue;

    gro_types ^= flag;
    pkt_count_fn = tbl_pkt_count_fn[i];
    if (pkt_count_fn) item_num += pkt_count_fn(gro_ctx->tbls[i]);
  }
