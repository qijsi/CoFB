...
static inline int merge_two_tcp4_packets(struct gro_tcp4_item *item,
  ...
  if (unlikely(pkt_head->pkt_len - l2_len + pkt_tail->pkt_len - hdr_len >
               MAX_IPV4_PKT_LENGTH)) {
    #if 0
    printf("pkt_head len:%d pkt_tail len:%d merged len:%d larger than %d\n",
           pkt_head->pkt_len, pkt_tail->pkt_len,
           pkt_head->pkt_len - l2_len + pkt_tail->pkt_len - hdr_len,
           MAX_IPV4_PKT_LENGTH);
    #endif
  ...
  item->nb_merged++;
  item->size = pkt_head->pkt_len - l2_len + pkt_tail->pkt_len - hdr_len;

  /* update MBUF metadata for the merged packet */
  pkt_head->nb_segs += pkt_tail->nb_segs;
  pkt_head->pkt_len += pkt_tail->pkt_len;
  //printf("merge two pkt, pkt_len: %d\n", pkt_head->pkt_len);
  return 1;
}
...
static inline int check_seq_option(struct gro_tcp4_item *item,
                                   struct tcp_hdr *tcph, uint32_t sent_seq,
  ...
  // qi: tcp_hl, tcp_hl_orig is 32, and sizeof(struct tcp_hdr) is 20, so len > 0
  len = RTE_MAX(tcp_hl - tcp_hl_orig, tcp_hl_orig - tcp_hl);

  if ((tcp_hl != tcp_hl_orig) ||
      ((len > 0) && (memcmp(tcph + 1, tcph_orig + 1, len) != 0))) {
    printf(
        "cannot merge, tcp header len not equal, tcp_hl:%u tcp_hl_orig:%u "
        "len:%u\n",
        tcp_hl, tcp_hl_orig, len);
    return 0;
  }

  /* Don't merge packets whose DF bits are different */
  if (unlikely(item->is_atomic ^ is_atomic)) {
    printf("cannot merge, DF bit is set\n");
    return 0;
  }
