...
typedef struct io_module_func {
	...
	uint8_t ** (*get_rptrv)(struct mtcp_thread_context *ctx, int ifidx, int index, uint16_t *len);
	int32_t   (*recv_pkts)(struct mtcp_thread_context *ctx, int ifidx);
	int32_t	  (*select)(struct mtcp_thread_context *ctx);
	uint8_t * (*get_mbuf)(struct mtcp_thread_context *ctx, int ifidx, int index);
...
int fdir_add_perfect_filter(int port_id, uint16_t soft_id, uint16_t rx_queue, uint32_t dip, \
		uint16_t dport, uint32_t sip, uint16_t sport);

int fdir_del_perfect_filter(int port_id, uint16_t soft_id, uint16_t rx_queue, uint32_t dip, \
		uint16_t dport, uint32_t sip, uint16_t sport);
