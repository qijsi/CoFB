...
// #define PROBECCP                               // print all cwnd changes,
// similar to tcpprobe output
#define CC_NAME 20
#endif
...
struct mtcp_config {
  ...
  int *nif_to_eidx;  // mapping physic port indexes to that of the configured
                     // port-list
  int eths_num;
...
struct mtcp_manager {
  ...
  int sp_fd;
  struct log_thread_context *logger;
  struct log_buff *w_buffer;
  FILE *log_fp;
...
struct mtcp_thread_context {
  ...
  uint64_t poll_id;
  uint64_t active_poll_id;
  struct timeval poll_time_point[2];
  struct timeval last_poll_time_point[2];
  struct timeval last_last_poll_time_point[2];
  struct mtcp_manager *mtcp_manager;

  void *io_private_context;
  #ifdef ENABLE_GRO
  void *io_private_context_large;
  #endif
...
{
	int cpu;
	struct mtcp_thread_context *mthc;
};
#endif
...
/*----------------------------------------------------------------------------*/
//extern pthread_spinlock_t fdir_ins_lock;
//extern pthread_spinlock_t fdir_del_lock;
extern pthread_spinlock_t fdir_lock;
//extern pthread_spinlock_t flow_lock;
