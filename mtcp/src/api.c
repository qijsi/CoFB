...
//	int rx_inf, ret;
	...
	for(rx_inf = 0; rx_inf < CONFIG.eths_num; rx_inf++) {
		ret = fdir_add_signature_filter(rx_inf, mtcp->ctx->cpu, addr_in->sin_addr.s_addr, addr_in->sin_port, 0, 0);
		if (ret < 0) {
			fprintf(stdout, "Failed to add master fdir filter\n");
			fflush(stdout);
	} else {
		fprintf(stdout, "Master add fdir filter: ip 0x%x port %d\n", addr_in->sin_addr.s_addr, addr_in->sin_port);
		fflush(stdout);
	}
}
}
...
mtcp_listen(mctx_t mctx, int sockid, int backlog)
{
	...
	if (ListenerHTSearch(mtcp->listeners, 
		...
		printf("listening on the same port\n");
		errno = EADDRINUSE;
		return -1;
	}
...
mtcp_accept(mctx_t mctx, int sockid, struct sockaddr *addr, socklen_t *addrlen)
{
	...
	ret = fdir_add_signature_filter(i, mtcp->ctx->cpu, accepted->saddr, \
			accepted->sport, accepted->daddr, accepted->dport);

	if (ret < 0) {
		fprintf(stdout, "failed to add signature filter: %s\n", strerror(-ret));
		fflush(stdout);
	} else {
		fprintf(stdout, "thread %d add filter: (little endian) dip 0x%x dport %u sip 0x%x sport %u\n", \
				mtcp->ctx->cpu, htonl(accepted->saddr), htons(accepted->sport), htonl(accepted->daddr), htons(accepted->dport));
		fflush(stdout);
	}
#endif
...
#if PROFILING
	fprintf(stdout, "%s %d lcore:%d sport:%u in mtcp_close\n", \
	...
	if (cur_stream->need_wnd_adv) {
		if (rcvvar->rcv_wnd > cur_stream->sndvar->eff_mss) {
			if (!cur_stream->sndvar->on_ackq) {
				...
				if(cur_stream->state > TCP_ST_ESTABLISHED) {
					fprintf(stdout, "%s %d lcore:%d sport:%u enqueue ackq\n", \
							__FILE__, __LINE__, mtcp->ctx->cpu, ntohs(cur_stream->dport));
					fflush(stdout);
				}
...
mtcp_recv(mctx_t mctx, int sockid, char *buf, size_t len, int flags)
{
	...
	cur_stream = socket->stream;
#if LATENCY_DUMP
	gettimeofday(&cur_stream->timestamp[1], NULL);
#endif
        if (!cur_stream || 
...
mtcp_write(mctx_t mctx, int sockid, const char *buf, size_t len)
{
	...
	cur_stream = socket->stream;
	gettimeofday(&cur_stream->sendtime[0], NULL);
	
#if LATENCY_DUMP
	gettimeofday(&cur_stream->timestamp[2], NULL);
#endif
	if (!cur_stream || 
			!(cur_stream->state == TCP_ST_ESTABLISHED || 
			  cur_stream->state == TCP_ST_CLOSE_WAIT)) {
	...
	if (ret > 0 && !(sndvar->on_sendq || sndvar->on_send_list)) {
		...
		if (cur_stream->state > TCP_ST_ESTABLISHED) {
			fprintf(stdout, "%s %d lcore:%d sport:%u mtcp_write\n",\
					__FILE__, __LINE__, mtcp->ctx->cpu, ntohs(cur_stream->dport));
			fflush(stdout);
		}
