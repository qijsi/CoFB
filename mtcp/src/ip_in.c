...
inline int 
ProcessIPv4PacketV(mtcp_manager_t mtcp, uint32_t cur_ts, 
				  const int ifidx, unsigned char** pkt_data, int len)
{
	/* check and process IPv4 packets */
	struct iphdr* iph = (struct iphdr *)(pkt_data[0] + sizeof(struct ethhdr));
	int ip_len = ntohs(iph->tot_len);
	int rc = -1;
	...
	switch (iph->protocol) {
		case IPPROTO_TCP:
			return ProcessTCPPacketV(mtcp, cur_ts, ifidx, iph, ip_len);
		case IPPROTO_ICMP:
			return ProcessICMPPacket(mtcp, iph, ip_len);
		default:
			/* currently drop other protocols */
