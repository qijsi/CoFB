...
/*----------------------------------------------------------------------------*/
static inline void GenerateTCPTimestamp(tcp_stream *cur_stream, uint8_t *tcpopt,
                                        uint32_t cur_ts) {
...
int SendTCPPacketStandalone(struct mtcp_manager *mtcp, uint32_t saddr,
  ...
  tcph = (struct tcphdr *)IPOutputStandalone(
      mtcp, IPPROTO_TCP, 0, saddr, daddr, TCP_HEADER_LEN + optlen + payloadlen);
  ...
  if (mtcp->iom->dev_ioctl != NULL)
    rc =
        mtcp->iom->dev_ioctl(mtcp->ctx, GetOutputInterface(daddr, &is_external),
                             PKT_TX_TCPIP_CSUM, NULL);
  UNUSED(is_external);
#endif
  if (rc == -1)
    tcph->check = TCPCalcChecksum(
        (uint16_t *)tcph, TCP_HEADER_LEN + optlen + payloadlen, saddr, daddr);
#endif
...
int SendTCPPacket(struct mtcp_manager *mtcp, tcp_stream *cur_stream,
                  uint32_t cur_ts, uint8_t flags, uint8_t *payload,
                  uint16_t payloadlen) {
  struct tcphdr *tcph;
  uint16_t optlen;
  uint8_t wscale = 0;
  ...
  gettimeofday(&cur_stream->timestamp[3], NULL);
  latency =
      (cur_stream->timestamp[3].tv_sec - cur_stream->timestamp[0].tv_sec) +
      (cur_stream->timestamp[3].tv_usec - cur_stream->timestamp[0].tv_usec) *
          0.000001;

  if (latency > 0.001) {
    fprintf(
        stdout, "%lf: %lf %lf %lf\n", latency,
        (cur_stream->timestamp[1].tv_sec - cur_stream->timestamp[0].tv_sec) +
            (cur_stream->timestamp[1].tv_usec -
             cur_stream->timestamp[0].tv_usec) *
                0.000001,
        (cur_stream->timestamp[2].tv_sec - cur_stream->timestamp[1].tv_sec) +
            (cur_stream->timestamp[2].tv_usec -
             cur_stream->timestamp[1].tv_usec) *
                0.000001,
        (cur_stream->timestamp[3].tv_sec - cur_stream->timestamp[2].tv_sec) +
            (cur_stream->timestamp[3].tv_usec -
             cur_stream->timestamp[2].tv_usec) *
                0.000001);
  ...
  if (flags & TCP_FLAG_WACK) {
    ...
    TRACE_CLWND(
        "%u Sending ACK to get new window advertisement. "
  ...
  if (rc == -1)
    tcph->check =
        TCPCalcChecksum((uint16_t *)tcph, TCP_HEADER_LEN + optlen + payloadlen,
  ...
  if (payloadlen > 0) {
    ...
    TRACE_RTO(
        "Updating retransmission timer. "
        "cur_ts: %u, rto: %u, ts_rto: %u\n",
        cur_ts, cur_stream->sndvar->rto, cur_stream->sndvar->ts_rto);

#if PROFILINGRTO
    fprintf(stdout,
            "%s %d lcore:%d sport:%u Updating retransmission timer. "
            "cur_ts: %u, rto: %u, ts_rto: %u\n",
...
/*----------------------------------------------------------------------------*/
static int FlushTCPSendingBuffer(mtcp_manager_t mtcp, tcp_stream *cur_stream,
                                 uint32_t cur_ts) {
#if 0
 ...
 out:
  ...
  while (1) {
...
#if USE_CCP
    // Without this, mm continually drops packets (not sure why, bursting?) ->
    // mtcp sees lots of losses -> throughput dies
    ...
    if (TCP_SEQ_LT(seq, sndvar->sndbuf->head_seq)) {
      TRACE_ERROR(
          "Stream %d: Invalid sequence to send. "
          "state: %s, seq: %u, head_seq: %u.\n",
          cur_stream->id, TCPStateToString(cur_stream), seq,
          sndvar->sndbuf->head_seq);
      assert(0);
      break;
    }
    ...
    remaining_window =
        MIN(sndvar->cwnd, sndvar->peer_wnd) - (seq - sndvar->snd_una);
      ...
      cur_stream->bucket->rate =
          (uint32_t)(SECONDS_TO_USECS(  // bits / s = mbps
              BYTES_TO_BITS(            // bits / us
                  (double)sndvar->cwnd /
                  UNSHIFT_SRTT(cur_stream->rcvvar->srtt)  // bytes / us
                  )));
    }
    if (cur_stream->bucket->rate != 0 &&
        (SufficientTokens(cur_stream->bucket, pkt_len * 8) < 0)) {
      packets = -3;
      goto out;
    }
#endif
...
#endif
    if ((sndlen = SendTCPPacket(mtcp, cur_stream, cur_ts, TCP_FLAG_ACK, data,
                                pkt_len)) < 0) {
...
#if 0
      gettimeofday(&cur_stream->sendtime[1], NULL);
      fprintf(stdout, "send time: %lu us\n", (cur_stream->sendtime[1].tv_sec - cur_stream->sendtime[0].tv_sec)*1000000 + \
      (cur_stream->sendtime[1].tv_usec - cur_stream->sendtime[0].tv_usec));
      fflush(stdout);
#endif
    packets++;
  }
...
/*----------------------------------------------------------------------------*/
static inline int SendControlPacket(mtcp_manager_t mtcp, tcp_stream *cur_stream,
                                    uint32_t cur_ts) {
  struct tcp_send_vars *sndvar = cur_stream->sndvar;
  int ret = 0;
  ...
  if (cur_stream->state == TCP_ST_SYN_SENT) {
    ...
    ret = SendTCPPacket(mtcp, cur_stream, cur_ts, TCP_FLAG_SYN | TCP_FLAG_ACK,
                        NULL, 0);
    ...
    if (sndvar->on_send_list || sndvar->on_ack_list) {
      ret = -1;
    } else if (!sndvar->is_fin_sent || (cur_ts > cur_stream->sndvar->ts_rto)) {
      /* Send FIN/ACK here */
      ret = SendTCPPacket(mtcp, cur_stream, cur_ts, TCP_FLAG_FIN | TCP_FLAG_ACK,
                          NULL, 0);

      fprintf(stdout, "sport:%u send resp in last_ack\n", htons(cur_stream->dport));
      fflush(stdout);
...
/*----------------------------------------------------------------------------*/
inline int WriteTCPControlList(mtcp_manager_t mtcp, struct mtcp_sender *sender,
  ...
  while (cur_stream) {
    ...
    if (cur_stream->sndvar->on_control_list) {
      ...
      if (ret == -2) {
        TAILQ_INSERT_HEAD(&sender->control_list, cur_stream,
                          sndvar->control_link);
        ...
        TAILQ_INSERT_TAIL(&sender->control_list, cur_stream,
                          sndvar->control_link);
        cur_stream->sndvar->on_control_list = TRUE;
        sender->control_list_cnt++;
      }
...
/*----------------------------------------------------------------------------*/
inline int WriteTCPDataList(mtcp_manager_t mtcp, struct mtcp_sender *sender,
                            uint32_t cur_ts, int thresh) {
  tcp_stream *cur_stream;
  tcp_stream *next, *last;
  int cnt = 0;
...
/*----------------------------------------------------------------------------*/
inline int WriteTCPACKList(mtcp_manager_t mtcp, struct mtcp_sender *sender,
                           uint32_t cur_ts, int thresh) {
  tcp_stream *cur_stream;
  tcp_stream *next, *last;
  int to_ack;
    ...
    if (cur_stream->sndvar->on_ack_list) {
      ...
      if (cur_stream->state == TCP_ST_ESTABLISHED ||
        ...
        TRACE_DBG(
            ...
            cur_stream->id, TCPStateToString(cur_stream), cur_stream->snd_nxt,
            cur_stream->rcv_nxt, cur_stream->sndvar->on_control_list);
#ifdef DUMP_STREAM
        DumpStream(mtcp, cur_stream);
#endif
      }
      ...
      if (cur_stream->control_list_waiting) {
        if (!cur_stream->sndvar->on_send_list) {
          cur_stream->control_list_waiting = FALSE;
          /*
          fprintf(stdout, "%s %d lcore:%d sport:%u to AddtoControlList\n", \
                          __FILE__, __LINE__, mtcp->ctx->cpu,
          ntohs(cur_stream->dport)); fflush(stdout);
          */
...
/*----------------------------------------------------------------------------*/
inline struct mtcp_sender *GetSender(mtcp_manager_t mtcp,
                                     tcp_stream *cur_stream) {
...
/*----------------------------------------------------------------------------*/
inline void AddtoControlList(mtcp_manager_t mtcp, tcp_stream *cur_stream,
                             uint32_t cur_ts) {
...
/*----------------------------------------------------------------------------*/
inline void EnqueueACK(mtcp_manager_t mtcp, tcp_stream *cur_stream,
