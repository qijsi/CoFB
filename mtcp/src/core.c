...
#define tmp_container_of(ptr, type, member)                                       \
  ({ \
...
#if defined(PKTDUMP) || (DBGMSG) || (DBGFUNC) || (STREAM) || (STATE) || \
    (STAT) || (APP) || (EPOLL) || (DUMP_STREAM)
...
/*----------------------------------------------------------------------------*/
struct rte_ring *conn_ring[MAX_CPUS];
/*----------------------------------------------------------------------------*/

typedef void (*app_ptr)(void *arg);
app_ptr app_logic, master_logic;

void register_appfunc(void (*func)(void *arg), bool master) {
  if (master)
    master_logic = func;
  else
    app_logic = func;
  ...
  if (signal == SIGINT) {
    ...
    if (CONFIG.multi_process) {
      for (i = 0; i < num_cpus; i++)
        if (running[i] == TRUE) g_pctx[i]->exit = TRUE;
    } else {
      if (sigint_cnt[core] > 0 && cur_ts.tv_sec > sigint_ts[core].tv_sec) {
        ...
        for (i = 0; i < num_cpus; i++) {
          if (running[i]) g_pctx[i]->interrupt = TRUE;
        }
...
/*----------------------------------------------------------------------------*/
static inline void UpdateStatCounter(struct stat_counter *counter,
                                     int64_t value) {
  counter->cnt++;
  counter->sum += value;
  if (value > counter->max) counter->max = value;
  if (counter->min == 0 || value < counter->min) counter->min = value;
}
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
static inline void PrintThreadNetworkStats(mtcp_manager_t mtcp,
                                           struct net_stat *ns) {
  int i;
...
/*----------------------------------------------------------------------------*/
#if ROUND_STAT
static inline void PrintThreadRoundStats(mtcp_manager_t mtcp,
                                         struct run_stat *rs) {
#define ROUND_DIV (1000)
  ...
  rs->rounds_rx_try =
      mtcp->runstat.rounds_rx_try - mtcp->p_runstat.rounds_rx_try;
  rs->rounds_tx = mtcp->runstat.rounds_tx - mtcp->p_runstat.rounds_tx;
  rs->rounds_tx_try =
      mtcp->runstat.rounds_tx_try - mtcp->p_runstat.rounds_tx_try;
  rs->rounds_select =
      mtcp->runstat.rounds_select - mtcp->p_runstat.rounds_select;
  rs->rounds_select_rx =
      mtcp->runstat.rounds_select_rx - mtcp->p_runstat.rounds_select_rx;
  rs->rounds_select_tx =
      mtcp->runstat.rounds_select_tx - mtcp->p_runstat.rounds_select_tx;
  rs->rounds_select_intr =
      mtcp->runstat.rounds_select_intr - mtcp->p_runstat.rounds_select_intr;
  rs->rounds_twcheck =
      mtcp->runstat.rounds_twcheck - mtcp->p_runstat.rounds_twcheck;
  ...
  fprintf(stderr,
          ...
          mtcp->ctx->cpu, rs->rounds / ROUND_DIV, rs->rounds_rx / ROUND_DIV,
          rs->rounds_rx_try / ROUND_DIV, rs->rounds_tx / ROUND_DIV,
          rs->rounds_tx_try / ROUND_DIV, rs->rounds_select,
...
static inline void PrintEventStat(int core, struct mtcp_epoll_stat *stat) {
  fprintf(stderr,
          ...
          core, stat->calls, stat->waits, stat->wakes, stat->issued,
          stat->registered, stat->invalidated, stat->handled);
  ...
  for (i = 0; i < CONFIG.eths_num; i++) {
    if (CONFIG.eths[i].stat_print) {
      fprintf(stderr,
              ...
              "RX: %7ld(pps) (err: %5ld), %5.2lf(Gbps), "
              "TX: %7ld(pps), %5.2lf(Gbps)\n",
              CONFIG.eths[i].dev_name, gflow_cnt, g_nstat.rx_packets[i],
              g_nstat.rx_errors[i], GBPS(g_nstat.rx_bytes[i]),
              g_nstat.tx_packets[i], GBPS(g_nstat.tx_bytes[i]));
    }
  }
#ifdef ENABLELRO
...
static inline void FlushEpollEvents(mtcp_manager_t mtcp, uint32_t cur_ts) {
  ...
  */
  ...
  if ((af - be)*1e9/rte_get_tsc_hz() > 5*1e4) {
          fprintf(stdout, "%s %d lcore:%d wait epoll lock too long\n", \
  ...
  */
  ...
  if (ep->mtcp_queue->num_events > 0) {
    ...
    while (mtcpq->num_events > 0 && usrq->num_events < usrq->size) {
      ...
      if (mtcpq->start >= mtcpq->size) mtcpq->start = 0;
      mtcpq->num_events--;
    }
  }
...
/*----------------------------------------------------------------------------*/
static inline void HandleApplicationCalls(mtcp_manager_t mtcp,
                                          uint32_t cur_ts) {
  tcp_stream *stream;
  int cnt, max_cnt;
  int handled, delayed;
  ...
  while ((stream = StreamDequeue(mtcp->sendq))) {
    ...
    if (stream->state > TCP_ST_ESTABLISHED) {
      fprintf(stdout,
              "%s %d lcore:%d sport:%u dequeue sendq, current state:%u\n",
              __FILE__, __LINE__, mtcp->ctx->cpu, ntohs(stream->dport),
              stream->state);
      fflush(stdout);
    }
  ...
  while ((stream = StreamDequeue(mtcp->ackq))) {
    ...
    if (stream->state == TCP_ST_LAST_ACK) {
      fprintf(stdout, "%s %d lcore:%d sport:%u dequeue ackq\n", __FILE__,
              __LINE__, mtcp->ctx->cpu, ntohs(stream->dport));
      fflush(stdout);
      continue;
    }
  ...
  while ((stream = StreamDequeue(mtcp->closeq))) {
...
#if PROFILING
      fprintf(stdout,
              "%s %d lcore:%d sport:%u state:%d fss(%u)=head_seq(%u)+len(%d)\n",
              __FILE__, __LINE__, mtcp->ctx->cpu, ntohs(stream->dport),
              stream->state, sndvar->fss, sndvar->sndbuf->head_seq,
              sndvar->sndbuf->len);
      fflush(stdout);
#endif
    } else {
...
#if PROFILING
      fprintf(stdout, "%s %d lcore:%d sport:%u state:%d fss=snd_nxt(%u)\n",
              __FILE__, __LINE__, mtcp->ctx->cpu, ntohs(stream->dport),
              stream->state, stream->snd_nxt);
      fflush(stdout);
#endif
    }

    if (CONFIG.tcp_timeout > 0) RemoveFromTimeoutList(mtcp, stream);
    ...
    if (stream->have_reset) {
      ...
      delayed++;
      if (sndvar->on_control_list) control++;
      if (sndvar->on_send_list) send++;
      if (sndvar->on_ack_list) ack++;
      ...
      if (stream->state == TCP_ST_ESTABLISHED) {
        ...
        fprintf(stdout, "Stream %d: change state to TCP_ST_LAST_ACK\n",
                stream->id);
        fflush(stdout);
      }
  ...
  while (cnt++ < max_cnt) {
    ...
    if (stream->sndvar->on_control_list || stream->sndvar->on_send_list ||
        stream->sndvar->on_ack_list) {
      /* wait until all the queues are flushed */
      StreamInternalEnqueue(mtcp->resetq_int, stream);
...
static inline void WritePacketsToChunks(mtcp_manager_t mtcp, uint32_t cur_ts) {
  ...
  int i;
#if 0
  if (mtcp->g_sender->send_list_cnt || mtcp->n_sender[0]->control_list_cnt) {
    printf("In WritePacketsToChunks\n");
  }
#endif
...
#if 0
    fprintf(stdout, "%s %d lcore:%d g_sender ack_list\n", __FILE__, __LINE__,
            mtcp->ctx->cpu);
    fflush(stdout);
#endif
...
volatile uint64_t coremask[LOAD_LEVEL];
volatile int worker_ready = 0;

/*----------------------------------------------------------------------------*/
// void RunWorkerLoop(struct mtcp_thread_context *ctx) {

__thread struct timeval latency[9];
__thread uint32_t latency_us[8];
__thread uint32_t conn_batch, pkt_batch;

void RunWorkerLoop(void *arg) {
  struct thread_context *tc = (struct thread_context *)arg;
  mctx_t t_mctx = tc->mctx;
  struct mtcp_thread_context *ctx = t_mctx->mthc;
  mtcp_manager_t mtcp = ctx->mtcp_manager;
  int i;
  int recv_cnt = 0, recv_conn = 0;
  int rx_inf, tx_inf;
  ...
  sprintf(ring_name, "%d", lcore);
  fprintf(stdout, "lcore:%d actualcore:%d\n", ctx->cpu, whichCoreID(ctx->cpu));
  fflush(stdout);

  *(conn_ring + lcore) = rte_ring_create(ring_name, 512, rte_socket_id(),
                                         RING_F_SC_DEQ | RING_F_SP_ENQ);

//  set_bit(lcore, &coremask[IDLE]);

  worker_ready++;

  while (worker_ready < (CONFIG.num_cores - 1)) {
    sleep(0.001);
  }
  ...
  while ((!ctx->done || mtcp->flow_cnt) && !ctx->exit) {
    ...
    for (rx_inf = 0; rx_inf < CONFIG.eths_num; rx_inf++) {
      ...
      mtcp->cur_ts = ts;
      latency[0] = cur_ts;
      recv_conn = rte_ring_dequeue_burst(conn_ring[lcore], (void **)&mbuf,
                                         BATCH_SIZE, NULL);
      conn_batch += recv_conn;
      for (i = 0; i < recv_conn; i++) {
        if (mbuf[i]) {
          gettimeofday(&cur_ts, NULL);
          ts = TIMEVAL_TO_TS(&cur_ts);
          mtcp->cur_ts = ts;
          pktbuf = rte_pktmbuf_mtod(mbuf[i], uint8_t *);
          if (pktbuf != NULL)
            ProcessPacket(mtcp, mbuf[i]->port, ts, pktbuf, mbuf[i]->pkt_len);
        }
#if 0
        printf("process one pkt from conn, control_list_cnt:%d\n",
               mtcp->n_sender[0]->control_list_cnt);
#endif
      }
      ...
      latency_us[0] += (latency[1].tv_sec - latency[0].tv_sec) * 1000000 +
                       (latency[1].tv_usec - latency[0].tv_usec);
      ...
      latency_us[1] += (latency[2].tv_sec - latency[1].tv_sec) * 1000000 +
                       (latency[2].tv_usec - latency[1].tv_usec);
...
#if 0
    latency_us[2] = (latency[3].tv_sec - latency[2].tv_sec) * 1000000 +
                    (latency[3].tv_usec - latency[2].tv_usec);
    latency_us[3] = (latency[4].tv_sec - latency[3].tv_sec) * 1000000 +
                    (latency[4].tv_usec - latency[3].tv_usec);
    latency_us[4] = (latency[5].tv_sec - latency[4].tv_sec) * 1000000 +
                    (latency[5].tv_usec - latency[4].tv_usec);
    latency_us[5] = (latency[6].tv_sec - latency[5].tv_sec) * 1000000 +
                    (latency[6].tv_usec - latency[5].tv_usec);
    latency_us[6] = (latency[7].tv_sec - latency[6].tv_sec) * 1000000 +
                    (latency[7].tv_usec - latency[6].tv_usec);
    latency_us[7] = (latency[8].tv_sec - latency[7].tv_sec) * 1000000 +
                    (latency[8].tv_usec - latency[7].tv_usec);
    if (latency_us[0] >= 200 || latency_us[1] >= 200 || latency_us[2] >= 200 ||
        latency_us[3] >= 200 || latency_us[4] >= 200 || latency_us[5] >= 200 ||
        latency_us[6] >= 200 || latency_us[7] >= 200) {
      fprintf(
          stdout,
          "long loop: conn batch: %u %u pkt batch: %u %u %u %u %u %u %u %u\n",
          conn_batch, latency_us[0], pkt_batch, latency_us[1], latency_us[2],
          latency_us[3], latency_us[4], latency_us[5], latency_us[6],
          latency_us[7]);
      fflush(stdout);
    }
  #endif
...
// void RunMasterLoop(struct mtcp_thread_context *ctx) {
void RunMasterLoop(void *arg) {
  struct thread_context *tc = (struct thread_context *)arg;

  mctx_t t_mctx = tc->mctx;
  struct mtcp_thread_context *ctx = t_mctx->mthc;

  // struct mtcp_thread_context *ctx = (struct mtcp_thread_context *)arg;
  mtcp_manager_t mtcp = ctx->mtcp_manager;
  ...
  TRACE_DBG("CPU %d: mtcp thread running.\n", ctx->cpu);
  // ts = TIMEVAL_TO_TS(&cur_ts);
  fprintf(stdout, "master:%d actualcore:%d\n", ctx->cpu, whichCoreID(ctx->cpu));
  fflush(stdout);
  ...
  while (!ctx->done && !ctx->exit) {
    ...
    for (rx_inf = 0; rx_inf < CONFIG.eths_num; rx_inf++) {
      ...
      STAT_COUNT(mtcp->runstat.rounds_rx_try);
#if 0
	  for(i=1;i<CONFIG.num_cores;i++) {
		  num_descriptor = rte_eth_rx_queue_count(rx_inf, i);
		  if (num_descriptor > 32) {
			  fprintf(stdout, "queue %u use %u descriptor\n", i, num_descriptor);
			  fflush(stdout);
		  }
	  }
#endif
...
/*----------------------------------------------------------------------------*/
void DestroyMTCPSender(struct mtcp_sender *sender) { free(sender); }
/*----------------------------------------------------------------------------*/
static mtcp_manager_t InitializeMTCPManager(struct mtcp_thread_context *ctx) {
  ...
  mtcp->listeners =
      CreateHashtable(HashListener, EqualListener, NUM_BINS_LISTENERS);
  if (!mtcp->listeners) {
  ...
  mtcp->rv_pool =
      MPCreate(pool_name, sizeof(struct tcp_recv_vars),
  ...
  mtcp->sv_pool =
      MPCreate(pool_name, sizeof(struct tcp_send_vars),
...
#else
  mtcp->flow_pool =
      MPCreate(sizeof(tcp_stream), sizeof(tcp_stream) * CONFIG.max_concurrency);
  if (!mtcp->flow_pool) {
    CTRACE_ERROR("Failed to allocate tcp flow pool.\n");
    return NULL;
  }
  mtcp->rv_pool =
      MPCreate(sizeof(struct tcp_recv_vars),
               sizeof(struct tcp_recv_vars) * CONFIG.max_concurrency);
  if (!mtcp->rv_pool) {
    CTRACE_ERROR("Failed to allocate tcp recv variable pool.\n");
    return NULL;
  ...
  mtcp->sv_pool =
      MPCreate(sizeof(struct tcp_send_vars),
               sizeof(struct tcp_send_vars) * CONFIG.max_concurrency);
  if (!mtcp->sv_pool) {
    CTRACE_ERROR("Failed to allocate tcp send variable pool.\n");
    return NULL;
...
#endif
  mtcp->rbm_snd =
      SBManagerCreate(mtcp, CONFIG.sndbuf_size, CONFIG.max_num_buffers);
  if (!mtcp->rbm_snd) {
  ...
  mtcp->rbm_rcv =
      RBManagerCreate(mtcp, CONFIG.rcvbuf_size, CONFIG.max_num_buffers);
  if (!mtcp->rbm_rcv) {
  ...
  mtcp->smap =
      (socket_map_t)calloc(CONFIG.max_concurrency, sizeof(struct socket_map));
  if (!mtcp->smap) {
    perror("calloc");
  ...
  fprintf(stdout, "lcore %d startup latency:%g ms\n", mtcp->ctx->cpu, (end-start)*1e3/rte_get_tsc_hz());
  fflush(stdout);
#endif
...
static void *CCPRecvLoopThread(void *arg) {
  ...
  while (!ctx->done && !ctx->exit) {
    do {
      bytes_recvd =
          recvfrom(mtcp->from_ccp, recvBuf, CCP_MAX_MSG_SIZE, 0, NULL, NULL);
      if (bytes_recvd <= 0) {
        if (bytes_recvd < 0) {
...
/* affinitize the thread to this core first */
  ...
  if (!ctx) {
    perror("error occurs during calloc");
  ...
  if (cpu == 0 &&
      pthread_create(&ccp_run_thread, NULL, CCPRunThread, (void *)mtcp) != 0) {
    TRACE_ERROR("Failed to create thread running CCP on cpu 0");
  }

  if (pthread_create(&ccp_recv_thread[cpu], NULL, CCPRecvLoopThread,
                     (void *)mtcp) != 0) {
  ...
  if (master == whichCoreID(cpu)) {
    ...
    if (unlikely(listener == NULL)) {
      fprintf(stdout, "Cannot find listener\n");
      fflush(stdout);
    }
...
mctx_t mtcp_create_context(int cpu) {
  ...
  if (cpu >= CONFIG.num_cores) {
    TRACE_ERROR(
        "Failed initialize new mtcp context. "
        "Requested cpu id %d exceed the number of cores %d configured to "
        "use.\n",
  ...
  if (g_logctx[cpu] != NULL) {
    TRACE_ERROR("%s for cpu:%d was already initialized before!\n", __FUNCTION__,
                cpu);
    return NULL;
  }
  ...
  /* initialize logger */
  g_logctx[cpu] =
      (struct log_thread_context *)calloc(1, sizeof(struct log_thread_context));
  if (!g_logctx[cpu]) {
    perror("calloc");
...
#if defined(PKTDUMP) || (DBGMSG) || (DBGFUNC) || (STREAM) || (STATE) || \
    (STAT) || (APP) || (EPOLL) || (DUMP_STREAM)
  if (pthread_create(&log_thread[cpu], NULL, ThreadLogMain,
                     (void *)g_logctx[cpu])) {
  ...
  if (mtcp_master < 0) {
    mtcp_master = cpu;
    TRACE_INFO("CPU %d (core:%d) is now the (master) worker thread.\n",
               mtcp_master, whichCoreID(mtcp_master));
  }
    ...
    for (i = 0; i < CONFIG.max_concurrency; i++) {
      if (mtcp->smap[i].socktype == MTCP_SOCK_STREAM) {
        TRACE_DBG("Closing remaining socket %d (%s)\n", i,
                  TCPStateToString(mtcp->smap[i].stream));
#ifdef DUMP_STREAM
...
int mtcp_setconf(const struct mtcp_conf *conf) {
  ...
  if (conf->num_cores > 0) CONFIG.num_cores = conf->num_cores;
  if (conf->max_concurrency > 0) CONFIG.max_concurrency = conf->max_concurrency;
  if (conf->max_num_buffers > 0) CONFIG.max_num_buffers = conf->max_num_buffers;
  if (conf->rcvbuf_size > 0) CONFIG.rcvbuf_size = conf->rcvbuf_size;
  if (conf->sndbuf_size > 0) CONFIG.sndbuf_size = conf->sndbuf_size;

  if (conf->tcp_timewait > 0) CONFIG.tcp_timewait = conf->tcp_timewait;
  if (conf->tcp_timeout > 0) CONFIG.tcp_timeout = conf->tcp_timeout;
...
/*----------------------------------------------------------------------------*/
uint64_t fdir_group[MAX_NIC][GROUP];  // 8000 / 64 = 125 < 128;
uint64_t fdir_elem[MAX_NIC][ELEM];    // Each bit for fdir_id in a group
pthread_spinlock_t fdir_lock;

int mtcp_init(const char *config_file) {
  int i, nic;
  int ret;
  // pthread_spin_init(&fdir_ins_lock, PTHREAD_PROCESS_PRIVATE);
  // pthread_spin_init(&fdir_del_lock, PTHREAD_PROCESS_PRIVATE);
  pthread_spin_init(&fdir_lock, PTHREAD_PROCESS_PRIVATE);
  // pthread_spin_init(&flow_lock, PTHREAD_PROCESS_PRIVATE);
  // fps_init();
  for (nic = 0; nic < MAX_NIC; nic++) {
    for (i = 0; i < GROUP - 1; i++) {
      fdir_group[nic][i] = 0xffffffffffffffff;
    }
    fdir_group[nic][GROUP - 1] = 0x1fffffffffffffff;

    for (i = 0; i < ELEM; i++) fdir_elem[nic][i] = 0xffffffffffffffff;
  }
  /* getting cpu and NIC */
  ...
  if (num_cpus > MAX_CPUS) {
    TRACE_ERROR(
        "You cannot run mTCP with more than %d cores due "
...
void mtcp_destroy() {
  ...
  for (i = 0; i < CONFIG.eths_num; i++) DestroyAddressPool(ap[i]);
