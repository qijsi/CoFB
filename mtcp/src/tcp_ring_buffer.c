...
/*----------------------------------------------------------------------------*/
int getseq(struct tcp_ring_buffer* buff, uint32_t seq) {
  struct fragment_ctx* iter;
 for (iter = buff->fctx; iter != NULL; iter = iter->next) {
   if (iter->seq == seq) return true;
 }
     return false;
}
	   ...
	   void* data, uint32_t len, uint32_t cur_seq)
{
	...
	if (buff->size < end_off) {
          fprintf(stdout, "buff->size < end_off\n");
          fflush(stdout);
          return -2;
    }
	...
	if (buff->head_seq != buff->fctx->seq) {
		printf("head_seq:%u fctx->seq:%u merged_len:%u\n", buff->head_seq, buff->fctx->seq, buff->merged_len);
	}
#endif
