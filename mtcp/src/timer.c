...
#if PROFILINGRTO
		fprintf(stdout, "%s %d lcore:%d sport:%u update ts_rto(sndvar->ts_rto:%u (cur_ts:%u + sndvar->rto:%u)\n", \
...
HandleRTO(mtcp_manager_t mtcp, uint32_t cur_ts, tcp_stream *cur_stream)
{
	...
	/*
    fprintf(stdout, "%s %d lcore:%d sport:%u Timeout! (cur_ts:%u cur_stream->sndvar->ts_rto:%u rto:%ums), snd_una: %u, snd_nxt: %u\n", 
			__FILE__, __LINE__, mtcp->ctx->cpu, ntohs(cur_stream->dport), cur_ts, cur_stream->sndvar->ts_rto, \
	...
	*/
	...
	if (cur_stream->sndvar->nrtx < TCP_MAX_RTX) {
		...
		if (cur_stream->state < TCP_ST_ESTABLISHED) {
			...
			printf("destroy%d in handlerto(<established)\n", ntohs(cur_stream->dport));
	...
	printf("timeer out\n");
	cur_stream->sndvar->cwnd = 10 * cur_stream->sndvar->mss;
    ...
    /*
		fprintf(stdout, "%s %d lcore:%d sport:%u HandleRTO\n",\
				__FILE__, __LINE__, mtcp->ctx->cpu, ntohs(cur_stream->dport));
		fflush(stdout);
    */
...
#if 0
			fprintf(stdout, "%s %d lcore:%d sport:%u process RTO\n", \
					__FILE__, __LINE__, mtcp->ctx->cpu, cur_stream->dport);
			fflush(stdout);
#endif
...
CheckRtmTimeout(mtcp_manager_t mtcp, uint32_t cur_ts, int thresh)
{
	...
	while (1) {
		...
		for (walk = TAILQ_FIRST(rto_list);
			...
			//qi
			if (cur_ts - walk->sndvar->ts_rto > TS_TO_MSEC(walk->sndvar->rto)) {
